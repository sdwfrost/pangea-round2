...
## sub.rate.model <- opts$options$model

## if (!(sub.rate.model %in% c("correlated", "discrete", "relaxed")))
##   {
##     cat("Model must be one of 'correlated', 'discrete', or 'relaxed'.\n")
##     quit(save="no", status=1)
##   }
## January 6: settings for a strict clock.
sub.rate.model <- "discrete"
strict.clock.ctrl <- chronos.control(nb.rate.cat=1)

#tree.file <- '~/wip/reshist/data/excluded_no_recombinants.RootToTip.tree'
#regr.file <- '~/wip/reshist/data/excluded_no_recombinants.RootToTip.out'
#out.file <- '~/wip/reshist/data/excluded_no_recombinants.PinTips.nwk'
...
## DEBUGGING
## tree.file <- "ML_tree.nex"
## regr.file <- "ML_clockify_coeffs.csv"
## out.file <- "ML_timetree.nwk"

rooted.tree <- read.nexus(tree.file)

regr.coeffs <- read.csv(regr.file, stringsAsFactors=FALSE)

## Get the known times from the tips and the estimated time of the
## root.
root.time <- as.double(regr.coeffs$x.intercept[1])


# tip.times <- NULL
# for (i in 1:length(rooted.tree$tip.label))
  # {
    # ## The tip labels look like
    # ## 591__type2_mut0_time2200
    # ## Extract the time.
    # tip.time <- as.numeric(sub(".+_(.+)", "\\1",
                               # rooted.tree$tip.label[i]))
    # tip.times <- rbind(tip.times,
                       # data.frame(index=i, name=rooted.tree$tip.label[i],
                                  # time=tip.time))
  # }

### is there a faster way to do this last step?
tip.dates <- sapply(rooted.tree$tip.label, function(x) as.integer(sub('.+_([0-9]+)$', '\\1', x)))

tip.times <- data.frame(index=1:length(tip.dates), name=rooted.tree$tip.label, time=tip.dates, row.names=1:length(tip.dates))

## The maximum time in the tree:
max.time <- max(tip.times$time)

## The minimum tip time in the tree:
min.tip.time <- min(tip.times$time)

## Check that the estimated root time is less than min.tip.time; if
## not, exit with an error code.
if (root.time >= min.tip.time)
  {
    cat("Error: root time (", root.time,
        ") is greater than minimum sampling time (",
        min.tip.time, ").\n", sep="")
    quit(save="no", status=1)
  }
...
## Now, prepare the table of calibration values needed by chronos.
## First, we prepare a table containing only the root.
calibrating.values <- makeChronosCalib(rooted.tree)
...
calibrating.values <- rbind(calibrating.values,
                            data.frame(node=tip.times$index,
                                       age.min=max.time - tip.times$time,
                                       age.max=max.time - tip.times$time,
                                       soft.bounds=FALSE))


dated.tree <-
  tryCatch(RLchronos(rooted.tree, 
  					lambda=1, 
...
write.tree(dated.tree, out.file)
