...
for(i in 1:numtrees){
  ...
  sq <- slist[[i]]
  if(slen[i]>=4){
    gag.o <- pml(njlist[[i]],as.phyDat(sq[,1:1440]),model="GTR",inv=0.01,k=4,shape=0.5)
    gag.pmllist[[idx]] <- gag.o
    pol.o <- pml(njlist[[i]],as.phyDat(sq[,1441:4284]),model="GTR",inv=0.01,k=4,shape=0.5)
    pol.pmllist[[idx]] <- pol.o
    env.o <- pml(njlist[[i]],as.phyDat(sq[,4285:6807]),model="GTR",inv=0.01,k=4,shape=0.5)
